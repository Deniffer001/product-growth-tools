...
runtime-drift.test.ts: vendored profile runtime drift coverage across publishable CLIs
...
- skill self-install: `gsc skill install` 只把包内 skill 复制到 `.agents/skills/gkit/<skill>`，不替代完整 skill manager
