...
export type GscServiceAccount = JWTInput &
  Readonly<{ type: "service_account"; client_email: string; private_key: string }>;

export function resolveGscServiceAccountPath(
  profile: LoadedProfile,
  configuredPath: string,
): string {
  if (configuredPath.trim().length === 0) {
    throw new ProfileError("invalid_profile", "GSC serviceAccountFile is empty.");
  }
  ...
  if (relation.startsWith("..") || isAbsolute(relation)) {
    throw new ProfileError(
      "invalid_profile",
      "Relative GSC serviceAccountFile paths must stay inside the profile support directory.",
    );
  }
  return resolvedPath;
}

export async function readGscServiceAccount(
  path: string,
  read: typeof readFile = readFile,
): Promise<{ credentials: GscServiceAccount; privateKey: string }> {
  let source: string;
  try {
    ...
    throw new ProfileError(
      "invalid_profile",
      "The configured GSC service-account file could not be read.",
    );
  }
  let value: unknown;
  try {
    ...
    throw new ProfileError(
      "invalid_profile",
      "The configured GSC service-account file is not valid JSON.",
    );
  }
  if (
    value === null ||
    ...
    throw new ProfileError(
      "invalid_profile",
      "GSC requires a service_account JSON file with client_email and private_key.",
    );
  }
  const credentials = Object.freeze(value as GscServiceAccount);
